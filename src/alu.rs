...
fn add(b : usize, p : usize, q : usize, c : bool, hb : usize, cb : usize) -> (usize, bool, bool){
    let c = c as usize;
    let m = (1 << b) - 1;
    let s = (p + q + c) & m;
...
fn sub(b : usize, p : usize, q : usize, c : bool, hb : usize, cb : usize) -> (usize, bool, bool){
    let c = c as usize;
    let m = (1 << b) - 1;
    let s = (p.wrapping_sub(q).wrapping_sub(c)) & m;
    let h = borrow(hb, p, q, c);
    let c = borrow(ch, p, q, c);
    let z == s == 0;
    (s, h, c, z)  
}
...
pub fn add16e(p : u16, q : u8, c : bool) -> (u8, bool, bool, bool){
